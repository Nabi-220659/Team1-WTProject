...
const API_BASE = 'http://localhost:8000/Backend/knowledge-hub';
...
// ── Search bar (debounced & explicit) ───────────────────────────────────────
const searchInput = document.getElementById('searchInput');
const searchIcon = document.querySelector('.search-icon');
let searchTimer;

function performSearch(explicit = false) {
  currentSearch = searchInput.value.trim();
  console.log('Searching for:', currentSearch);
  fetchArticles(currentCategory, currentSearch, explicit);
}
...
if (searchInput) {
  ...
  searchInput.addEventListener('input', () => {
    ...
    searchTimer = setTimeout(() => performSearch(false), 350);
  });
